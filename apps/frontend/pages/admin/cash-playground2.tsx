...
export default function AdminCashPlayground2Page() {
  ...
  useEffect(() => {
    ...
    void router.replace(`/login?from=${encodeURIComponent(router.asPath || '/admin/cash-playground2')}`);
  ...
  return (
    <>
      <Head>
        <title>Caja Playground 2 | TuCancha Admin</title>
      </Head>

      <AdminPlaygroundShell activeItem="Pagos" user={user} contentMuted={actionSidebarOpen}>
        <div className="flex h-full min-h-0 flex-col gap-4 p-4 lg:p-6">
              <header className="rounded-xl border border-[#dce2ee] bg-white px-4 py-3 shadow-[0_8px_26px_rgba(34,42,68,0.05)]">
                <div className="flex flex-wrap items-center gap-3 md:gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h1 className="truncate text-base font-semibold text-[#1f2638]">Caja Operativa</h1>
                    <p className="text-[12px] text-[#6f7890]">Nueva experiencia visual, reutilizando la misma logica y endpoints.</p>
                  </div>
                  ...
                  <div className="flex items-center gap-1 rounded-xl border border-[#dce2ee] bg-white p-1">
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold transition ${
                        activeView === 'live' ? 'bg-[#edf1ff] text-[#3053e2]' : 'text-[#6f7890] hover:bg-[#f8f9fd]'
                      }`}
                    >
                      Caja en vivo
                    </button>
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold transition ${
                        activeView === 'movements' ? 'bg-[#edf1ff] text-[#3053e2]' : 'text-[#6f7890] hover:bg-[#f8f9fd]'
                      }`}
                    >
                      Movimientos
                    </button>
                    <button
                      ...
                      className={`rounded-lg px-3 py-1.5 text-[12px] font-semibold transition ${
                        activeView === 'closures' ? 'bg-[#edf1ff] text-[#3053e2]' : 'text-[#6f7890] hover:bg-[#f8f9fd]'
                      }`}
                    >
                      Cierres
                    </button>
                  ...
                  <button
                    ...
                    className="h-8 rounded-xl border border-[#dce2ee] bg-white px-3 text-[12px] font-semibold text-[#4e5870] transition hover:bg-[#f8f9fd]"
                  >
                  ...
                  <div className="flex items-center gap-1 rounded-xl border border-[#dce2ee] bg-white px-1 py-1">
                    ...
                    <span className="min-w-[120px] text-center text-[12px] font-semibold text-[#4e5870]">{periodLabel}</span>
                    <button
                      type="button"
                      onClick={() => setPeriodOffset((prev) => prev + 1)}
                      className="grid h-8 w-8 place-items-center rounded-lg text-[#6f7890] transition hover:bg-[#f4f6fb]"
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
                <article className={`rounded-xl border border-[#dce2ee] bg-white ${denseMode ? 'p-3' : 'p-4'}`}>
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-[#6f7890]">Saldo Total</p>
                  <p className={`mt-2 font-semibold text-[#1f2638] ${denseMode ? 'text-lg' : 'text-xl'}`}>{formatMoney(balance.total)}</p>
                </article>
                <article className={`rounded-xl border border-[#dce2ee] bg-white ${denseMode ? 'p-3' : 'p-4'}`}>
                ...
                <article className={`rounded-xl border border-[#dce2ee] bg-white ${denseMode ? 'p-3' : 'p-4'}`}>
                  ...
                  <p className={`mt-2 font-semibold text-[#1f2638] ${denseMode ? 'text-lg' : 'text-xl'}`}>{formatMoney(balance.cash)}</p>
                </article>
                <article className={`rounded-xl border border-[#dce2ee] bg-white ${denseMode ? 'p-3' : 'p-4'}`}>
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-[#6f7890]">Caja Digital</p>
                  <p className={`mt-2 font-semibold text-[#1f2638] ${denseMode ? 'text-lg' : 'text-xl'}`}>{formatMoney(balance.digital)}</p>
                </article>
              </div>
              ...
              {(summaryError || shiftError || movementError || successMessage) && (
                <div className="space-y-2">
                  ...
                  {summaryError && <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[12px] text-red-800">{summaryError}</div>}
                  {shiftError && <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[12px] text-red-800">{shiftError}</div>}
                  {movementError && <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[12px] text-red-800">{movementError}</div>}
                </div>
              )}
              ...
              <div className="min-h-0 flex-1 overflow-auto">
                    ...
                    <article className="min-h-0 rounded-xl border border-[#dce2ee] bg-white p-4">
                      ...
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div className="rounded-xl border border-[#dce2ee] bg-[#f8f9fd] px-3 py-3">
                          ...
                          <p className={`mt-1 inline-flex rounded-full px-2 py-1 text-[12px] font-semibold ${currentShift ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700'}`}>
                ...
                {activeView === 'movements' && (
                  <article className="min-h-0 rounded-xl border border-[#dce2ee] bg-white p-4">
                    ...
                    <div className={`overflow-auto rounded-xl border border-[#dce2ee] ${denseMode ? 'max-h-[68vh]' : 'max-h-[62vh]'}`}>
                      {filteredMovements.length === 0 ? (
                        ...
                        <table className={`w-full min-w-[680px] ${denseMode ? 'text-[13px]' : 'text-[13px]'}`}>
                          ...
                          <tbody>
                            {filteredMovements.map((movement) => (
                              <tr key={movement.id} className="border-t border-[#edf0f6] transition hover:bg-[#f4f6fb]">
                                <td className={`px-3 text-[12px] text-[#4e5870] ${denseMode ? 'py-1.5' : 'py-2'}`}>{formatDateTime24(movement.date)}</td>
                                <td className={`px-3 text-[#1f2638] ${denseMode ? 'py-1.5' : 'py-2'}`}>{movement.description}</td>
                                <td className={`px-3 text-[#4e5870] ${denseMode ? 'py-1.5' : 'py-2'}`}>{movementMethodLabel(movement.method)}</td>
                ...
                {activeView === 'closures' && (
                  <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                    ...
                    <article className="rounded-xl border border-[#dce2ee] bg-white p-4">
                      <h2 className="text-[13px] font-semibold text-[#1f2638]">Ultimo arqueo generado</h2>
                      {lastCloseReport ? (
                        <>
                        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                          <div className="rounded-xl border border-[#dce2ee] bg-[#f8f9fd] px-3 py-2">
                            <p className="text-[10px] uppercase tracking-wide text-[#6f7890]">Esperado</p>
                            <p className="mt-1 text-[13px] font-semibold text-[#1f2638]">{formatMoney(lastCloseReport.expectedCash)}</p>
                          </div>
                          <div className="rounded-xl border border-[#dce2ee] bg-[#f8f9fd] px-3 py-2">
                            <p className="text-[10px] uppercase tracking-wide text-[#6f7890]">Contado</p>
                            <p className="mt-1 text-[13px] font-semibold text-[#1f2638]">{formatMoney(lastCloseReport.countedCash)}</p>
                          </div>
                          <div className="rounded-xl border border-[#dce2ee] bg-[#f8f9fd] px-3 py-2">
                            <p className="text-[10px] uppercase tracking-wide text-[#6f7890]">Diferencia</p>
                            <p className={`mt-1 text-[13px] font-semibold ${Number(lastCloseReport.difference || 0) >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                        ...
                        <p className="mt-3 text-[13px] text-[#6f7890]">Aun no hay un arqueo generado en esta sesion.</p>
                      )}
                    </article>
                  </div>
                )}
      ...
      >
        <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
              ...
              <form className="grid grid-cols-1 gap-3" onSubmit={handleCreateMovement}>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <select
                    ...
                    className="h-10 rounded-xl border border-[#dce2ee] bg-white px-3 text-[13px] outline-none focus:border-[#3053e2]"
                  >
                ...
                <button
                  ...
                  className="h-10 rounded-xl bg-[#3053e2] px-4 text-[13px] font-semibold text-white transition hover:bg-[#2748cc] disabled:cursor-not-allowed disabled:opacity-60"
                >
            ...
            {sidebarView !== 'close_report' && (
              <section className="mt-4 rounded-xl border border-[#dce2ee] bg-[#f8f9fd] p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-[#6f7890]">Flujo recomendado</p>
                <ol className="mt-2 space-y-1 text-[12px] text-[#4e5870]">
